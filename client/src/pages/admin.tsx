...
export default function AdminPage() {
  ...
  const updateUser = useMutation({
    mutationFn: async (payload: { email: string; role?: string; active?: boolean; password?: string }) => {
    ...
    <div className="min-h-screen bg-[hsl(225_10%_6%)] text-white">
      ...
      <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Total de usuários", value: String(users.length), icon: Users, color: "blue" },
            { label: "Administradores", value: String(users.filter(u => u.role === "admin").length), icon: Shield, color: "violet" },
            { label: "Usuários ativos", value: String(users.filter(u => u.active).length), icon: Check, color: "emerald" },
            { label: "Versão", value: "1.0.0", icon: Info, color: "slate" },
          ...
          {showAdd && (
            <div className="px-6 py-4 border-b border-white/[0.06] bg-blue-500/[0.03]">
              ...
              <div className="flex items-center gap-2 mt-3">
                ...
                <button
                  onClick={() => { setShowAdd(false); setNewEmail(""); setNewName(""); setNewPass(""); }}
                      ...
                      <td className="px-6 py-3.5">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          {u.email !== myEmail && (
                            <button
                              onClick={() => {
                                if (confirm(`Remover ${u.email}?`)) deleteUser.mutate(u.email);
                              }}
                              title="Remover"
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-[hsl(222_9%_9%)] overflow-hidden">
          ...
          <div className="px-6 py-5 grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm">
            {[
              ...
              { label: "Fonte primária", value: "Portal da Transparência + CNJ DataJud" },
        ...
        <div className="flex items-start gap-2.5 px-4 py-3 rounded-xl bg-amber-500/[0.06] border border-amber-500/20 text-xs text-amber-300/80">
          ...
          <p>
            ...
            Para persistência, é necessário conectar um banco de dados.
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[hsl(222_9%_9%)] rounded-2xl border border-white/[0.1] shadow-2xl p-6">
            ...
            <div className="flex items-center justify-end gap-2 mt-5">
              ...
              <button
                onClick={() => {
                  const payload: any = { email: editTarget.email, role: editRole };
                  if (editPass.length >= 6) payload.password = editPass;
              ...
              >
                {updateUser.isPending ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                Salvar
              </button>
            </div>
          </div>
