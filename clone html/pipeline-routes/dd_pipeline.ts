...
async function executarFaseEnriquecimento(
  ...
  try {
    ...
    // ── PASSO 3: Tem CNPJ? → Tentar TRF1 consultarPorCNPJ ────────────────
    if (cnpjEnt && cnpjEnt.length >= 11) {
      resultado.metodo = "cnpj_para_trf1";
      console.log(`[Enriquecimento] CNJ ausente. Tentando via CNPJ ${cnpjEnt} no TRF1...`);

      try {
        const roboPje = require("../scripts/robo_pje/drivers/trf1.cjs");
        const trf1Result = await roboPje.consultarPorCNPJ(cnpjEnt, { headless: true, timeout: 45000 });

        if (trf1Result.processos && trf1Result.processos.length > 0) {
          resultado.candidatos_trf1 = trf1Result.processos;
          console.log(`[Enriquecimento] TRF1 retornou ${trf1Result.processos.length} CNJs para CNPJ ${cnpjEnt}`);
          ...
          if (trf1Result.processos.length === 1) {
            ...
            resultado.metodo = "cnpj_trf1_unico";
          }
          // Se encontrou poucos (<=10), retornar como candidatos para seleção
          // Futuramente: ranquear por valor, tipo_causa, etc.
        } else {
          resultado.metodo = "cnpj_trf1_sem_resultado";
          console.log(`[Enriquecimento] TRF1 não retornou processos para CNPJ ${cnpjEnt}`);
        }

        if (trf1Result.erro) {
          resultado.metodo = "cnpj_trf1_erro";
          console.log(`[Enriquecimento] Erro TRF1: ${trf1Result.erro}`);
        }
      ...
      return {
        ...
        fontes: [
          ...
          ...(resultado.candidatos_trf1.length > 0 ? [`TRF1 Consulta por CNPJ (${resultado.candidatos_trf1.length} processos)`] : []),
        ],
        timestamp: new Date().toISOString(),
        duracao_ms: Date.now() - inicio,
      };
