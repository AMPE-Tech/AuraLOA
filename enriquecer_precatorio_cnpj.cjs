...
// ── Fase 3 G2 — Cadeia de custódia via EvidencePack standalone ──
// Single source of truth: server/services/evidence_pack.ts → bundled em
// dist/lib/evidence_pack.cjs por script/build.ts (rodar `npm run build` antes).
// Ref: contrato_tecnico/aditivos/aditivo_2026-04-24_fase2.md G2.
const EVIDENCE_PACK_PATH = path.resolve(__dirname, 'dist/lib/evidence_pack.cjs');
let EvidencePack = null;
try {
  ({ EvidencePack } = require(EVIDENCE_PACK_PATH));
} catch (e) {
  console.error('[enriquecer] FATAL: dist/lib/evidence_pack.cjs nao encontrado.');
  console.error('[enriquecer] Rode `npm run build` antes de executar este script.');
  console.error('[enriquecer] Caminho esperado:', EVIDENCE_PACK_PATH);
  process.exit(1);
}
...
(async () => {
  ...
  // Fase 3 G2 — pack de evidência por execução
  const pack = new EvidencePack(packIdFor('enriquecer_cnpj', `${NOME}_${CNPJ}`));
  pack.log(`enriquecer start cnpj=${CNPJ} nome=${NOME}`);
  pack.saveRequest({ cnpj: CNPJ, nome: NOME, url: URL_BUSCA, headless: false, ts: new Date().toISOString() });

  console.log(`\n========================================`);
  console.log(`ENRIQUECIMENTO VIA CNPJ — ${NOME}`);
  console.log(`CNPJ: ${CNPJ}`);
  console.log(`PACK: ${pack.getBasePath()}`);
  console.log(`========================================\n`);
  ...
  pack.log(`extracao concluida: ${processos.length} processos em ${paginaAtual} pagina(s)`);
  ...
  // Fase 3 G2 — gravar CSV no pack para audit trail
  const savedCsv = pack.saveRawPayload(`${NOME}_processos_trf1.csv`, csvContent);
  pack.log(`csv salvo: ${csvPath} sha256=${savedCsv.sha256}`);
  ...
  console.log('\n[*] Aguardando 30s para inspeção...');
  await page.waitForTimeout(30000);
  await browser.close();

  // Fase 3 G2 — cadeia de custódia final
  const responseHash = pack.saveResponse({
    cnpj: CNPJ, nome: NOME, total_processos: processos.length,
    paginas: paginaAtual, csv_path: csvPath, csv_sha256: savedCsv.sha256,
    processos_amostra: processos.slice(0, 5),
  });
  pack.saveLog();
  console.log(`\n[evidence] processId=${pack.getBasePath().split(/[\\/]/).pop()} responseHash=${responseHash}`);
